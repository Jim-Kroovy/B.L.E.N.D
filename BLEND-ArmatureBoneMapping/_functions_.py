...
def Get_Bone_Indices_From_Name(name):
    print(name)

def Set_Mapping_Bone_Data(rig, template):
    for bone in rig.data.bones:
        bone.JK_MMT['type'] = 'DEFORM'
        bone.JK_MMT['matrix'] = bone.matrix_local.copy()
        bone.JK_MMT['parent'] = bone.parent.name if bone.parent != None else None
        bone.JK_MMT['indices'] = (-1, -1, -1) #Get_Bone_Indices_From_Name(bone.name)
        bone.JK_MMT['retarget'] = 'ANIMATION'
...
def Write_Mapping(mapping):
    ...
    text.write("""# then when we run this to set the mapping after updating...
    mapping.clear()
